...
# delete request
# example:  http://127.0.0.1:8000/books/delete_book/title%20four
...
"""
# example:  http://127.0.0.1:8000/books/byauthor/author%20two
...
# this won't work unless you move it above the
# @app.get("/books/{book_author}/")
# order matters. ㄷput smaller path before the bigger path
"""
@app.get("/books/byauthor/")  # without dynamic path parameter
async def read_books_by_author_path(
    author: str,
):  # so this will automatically be a query parameter
    books_to_return = []
    for book in BOOKS:
        if book.get("author").casefold() == author.casefold():
            books_to_return.append(book)
